...
describe("JettonWallet", () => {
  ...
  let estimateAdminTransferFwd: (amount: bigint, custom_payload: Cell | null, forward_amount: bigint, forward_payload: Cell | null, prices?: MsgPrices) => bigint;
  ...
  let testAdminTransfer: (ton_amount: bigint, transfer_amount: bigint, from: Address, fwd_amount: bigint, fwd: Cell | null, custom: Cell | null, exp: number) => Promise<void>;
  let testAdminBurn: (
    ton_amount: bigint,
    burn_amount: bigint,
    burn_addr: Address,
  ...
  beforeAll(async () => {
    ...
    estimateAdminTransferFwd = (jetton_amount, custom_payload, forward_amount, forward_payload, prices) => {
      const mockAddr = randomAddress(0);
      const curPrices = prices || msgPrices;
      const body = JettonMinter.forceTransferMessage(jetton_amount, mockAddr, mockAddr, custom_payload, forward_amount, forward_payload);
    ...
    testAdminTransfer = async (ton_amount, jetton_amount, from, fwd_amount, fwd, custom, exp) => {
      ...
      const sendResult = await jettonMinter.sendForceTransfer(deployer.getSender(), jetton_amount, to, from, custom, fwd_amount, fwd, ton_amount);
      expect(sendResult.transactions).toHaveTransaction({
        on: jettonMinter.address,
        from: deployer.address,
        op: Op.call_to,
    ...
    testAdminBurn = async (ton_amount, burn_amount, burn_addr, response_addr, cuntom_payload, exp) => {
      ...
      const res = await jettonMinter.sendForceBurn(deployer.getSender(), burn_amount, burn_addr, response_addr, ton_amount);
      let burnTxs: Array<BlockchainTransaction> = [];
  ...
  describe("Content tests", () => {
    ...
    it("minter admin can change content", async () => {
      ...
      expect(oldContent.get(await sha256("decimals"))! === "0").toBe(true);
    ...
    it("not a minter admin can not change content", async () => {
      ...
      expect(oldContent.get(await sha256("decimals"))).toEqual("0");
...
*/
  ...
  describe("Locking", () => {
    ...
    it("out locked wallet should be able to receive jettons", async () => {
      ...
      await deployerJettonWallet.sendTransfer(deployer.getSender(), toNano("1"), 1n, notDeployer.address, deployer.address, null, toNano("0.05"), null);
    ...
    it("in and full locked wallet should not be able to receive jettons", async () => {
      ...
      await testCap(["in", "full"], notDeployer.address, async () => {
        ...
        let res = await deployerJettonWallet.sendTransfer(deployer.getSender(), toNano("1"), 1n, notDeployer.address, deployer.address, null, toNano("0.05"), null);
        expect(res.transactions).toHaveTransaction({
          on: notDeployerJettonWallet.address,
          op: Op.internal_transfer,
          success: false,
    ...
    describe("Force transfer", () => {
      ...
      it("admin should be able to force jetton transfer", async () => {
        ...
        const res = await jettonMinter.sendForceTransfer(deployer.getSender(), txAmount, deployer.address, notDeployer.address, customPayload, fwdAmount, fwdPayload, msgValue);
    ...
    describe("Force burn", () => {
      ...
      it("admin should be able to force burn jettons in arbitrary wallet", async () => {
        ...
        await testAdminBurn(msgValue, burnAmount, notDeployer.address, deployer.address, null, 0);
      });
